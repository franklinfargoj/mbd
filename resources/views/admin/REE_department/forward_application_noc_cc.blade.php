                            ...
                            <div class="portlet-body">
                                <div class="m-portlet__body m-portlet__body--table m-portlet__body--serial-no m-portlet__body--serial-no-pdf">
                                    ...
                                    @if(count($reeLogs) == 0 && count($coLogs) == 0)
                                        <h4 class="section-title section-title--small mb-4">
                                            <small>No Remarks found..</small>
                                        </h4>
                                    @endif
                                    @if(count($reeLogs) > 0)
                                                    ...
                                                    @foreach($reeLogs as $log)
                                                        ...
                                                        <div class="remarks-section__data">
                                                            <p class="remarks-section__data__row">
                                                                <span>Date:</span><span>{{(isset($log) && $log->created_at != '' ? date("d-m-Y",
                                       strtotime($log->created_at)) : '')}}</span>
                                                            </p>
                                                            <p class="remarks-section__data__row">
                                                                <span>Time:</span><span>{{(isset($log) && $log->created_at != '' ? date("H:i",
                                       strtotime($log->created_at)) : '')}}</span>
                                                            </p>
                                                            <p class="remarks-section__data__row"><span>Action:</span>
                                                                <span>{{$status}}
                                                                    to {{isset($log->getRoleName->display_name) ? $log->getRoleName->display_name : ''}}
                                                                    From {{isset($log->getRole->display_name) ? $log->getRole->display_name : ''}}</span>
                                                            </p>
                                                            <p class="remarks-section__data__row">
                                                                <span>Description:</span><span>{{(isset($log) ? $log->remark : '')}}</span>
                                                            </p>
                                                        </div>
                                                    @endforeach
                                                </div>
                              ...
                              REE</span>
                                       ...
                                       strtotime($log->created_at)) : '')}}</span>
                                                        ...
                                                        <p class="remarks-section__data__row"><span>Action:</span>
                                                            <span>{{$status}}
                                                                to {{isset($log->getRoleName->display_name) ? $log->getRoleName->display_name : ''}}</span>
                                                        </p>
                                       ...
                                       to REE</span>
                                                    ...
                                                    <p class="remarks-section__data__row">
                                                        <span>Description:</span><span>{{(isset($applicationData->dyceForwardLog->remark)
                        ...
                        <div class="portlet-body">
                            <div class="m-portlet__body m-portlet__body--table m-portlet__body--serial-no m-portlet__body--serial-no-pdf">
                                ...
                                <div class="remarks-suggestions">
                                    <form action="{{ route('ree.forward_noc_cc_application_data') }}"
                                        ...
                                        <div class="m-form__group form-group">
                                            ...
                                            <div id="forwardapp" style="display:none;" class="form-group m-form__group row mt-3 parent-data"
                                                 id="select_dropdown">
...
@section('js')
    <script>
        $(document).ready(function () {
            ...
            if($('#forward:checked').val()) {
                    $('#forwardapp').show();
                    $('#revertapp').hide();
            }

            $('input[type="radio"]').click(function() {
                if($(this).attr('id') == 'sent_to_society') {
