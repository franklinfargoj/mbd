...
<li id="cap-actions" class="collapse show">
    <ul class="list-unstyled">
        <li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{($route=='architect_layout_details.view')?'m-menu__item--active':''}}">
        ...
        $check_layout_details_complete_status = count($CommonController->check_layout_details_complete_status($ArchitectLayout->id));
        @endphp
        @if($check_layout_details_complete_status==0 && (session()->get('role_name')==config('commanConfig.junior_architect')))
        <li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{( $route=='architect_layout_detail.edit')?'m-menu__item--active':''}}">
