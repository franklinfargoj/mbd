...
@section('content')
    <div class="col-md-12">
        ...
        <div class="m-portlet m-portlet--mobile m-portlet--forms-view">
            <form id="update_profile" role="form" method="post" class="m-form m-form--rows m-form--label-align-right floating-labels-form" action="{{ route('admin.update_profile') }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="m-portlet__body m-portlet__body--spaced">
                    @for($i=0; $i < count($field_names); $i++)
                        ...
                        <div class="form-group m-form__group row mhada-lease-margin">
                            ...
                            @if($field_names[$i] == 'password' || $field_names[$i] == 'confirm_password') 

                            @php if($field_names[$i] == 'password'){ $field_names[$i] = 'new_password'; } $type = 'password'; $value = ''; @endphp 
                            @else @php $type = 'text'; @endphp @endif
                                @if($field_names[$i] == 'id') @php $value = encrypt($users->id); $type = 'hidden'; @endphp @php echo $comm_func->form_fields($field_names[$i], $type, '', '', $value, '', 'required') @endphp 
                            @endif
