	...
	<ul class="list-unstyled">
		...
		@if($status == config('commanConfig.applicationStatus.pending'))
			@if((isset($applicationCount) && $applicationCount <= 0) || $route=='society_offer_letter_preview' || $route=='documents_uploaded' || ($route=='society_offer_letter_edit') || $route=='documents_upload' ||$route=='upload_multiple_documents')
				...
				<li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{($route=='documents_upload' || $route == 'upload_multiple_documents' || $route == 'upload_other_documents')?'m-menu__item--active':''}}">
		...
		@if($status == config('commanConfig.applicationStatus.forwarded') || $status == config('commanConfig.applicationStatus.sent_to_society') || $status == config('commanConfig.applicationStatus.Rejected') || ($status == config('commanConfig.applicationStatus.pending') && isset($documents_arr) && $documents_arr['docs_count'] == $documents_arr['docs_uploaded_count']) && isset($applicationCount) && $applicationCount != 0)
			...
			<li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{($route=='documents_uploaded' || $route == 'upload_other_documents')?'m-menu__item--active':''}}">
