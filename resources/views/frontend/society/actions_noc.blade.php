	...
	<ul class="list-unstyled">
		...
		@if($status == config('commanConfig.applicationStatus.pending') || $status == config('commanConfig.applicationStatus.reverted'))
			...
			@if(isset($applicationCount) && $applicationCount <= 0)
				...
				<li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{($route=='documents_upload_noc' || $route == 'upload_noc_other_documents')?'m-menu__item--active':''}}">
		...
		@if($status == config('commanConfig.applicationStatus.forwarded') || $status == config('commanConfig.applicationStatus.sent_to_society') )
			...
			<li class="m-menu__item m-menu__item--submenu m-menu__item--level-2 {{($route=='documents_uploaded_noc' || $route == 'upload_noc_other_documents')?'m-menu__item--active':''}}">
