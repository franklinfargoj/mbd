    ...
    <div class="m-subheader px-0 m-subheader--top">
        <div class="d-flex align-items-center">
            <h3 class="m-subheader__title m-subheader__title--separator">Upload documents</h3>
             ...
             @if($sc_application->scApplicationLog->status_id != config('commanConfig.applicationStatus.forwarded'))
            <a href="{{ route('sc_upload_docs',encrypt($sc_application->id)) }}" class="btn btn-link ml-auto"><i class="fa fa-long-arrow-left" style="padding-right: 8px;"></i>Back</a>
            @else
                ...
                <table class="mt-2 table table-hover" id="dtBasicExample"> 
                    ...
                    <tbody>
                            ...
                            <tr>
                                <td>{{ $i }}</td>
                                <td> 
                                    {{ isset($document->other_document_name) ? $document->other_document_name : '' }}
                                </td>
                                <td class=""> 
                                ...
                                @if($sc_application->scApplicationLog->status_id != config('commanConfig.applicationStatus.forwarded'))
                                    <td>
                                        ...
                                        <input type="hidden" name= "oldFile" id="oldFile_{{$document->id}}" value="{{ isset($document->document_path) ? $document->document_path : '' }}"> 
                                    </td>
                                @endif    
                            </tr>
                            @php $i++;@endphp 
...
<script type="text/javascript">
    ...
    $("#uploadBtn").click(function(){
        if ($("#myFrm").valid() == true){
            ...
            var societyId = '<?php echo $sc_application->society_id; ?>';
            var applicationId = '<?php echo $sc_application->id; ?>';               
            ...
            $.ajax({
                url: "/society/save_sc_other_documents",
                data: form_data,
                dataType : "json",
                type: 'POST',
                contentType: false, 
    ...
    function deleteDocument(id){
        ...
        if (valid){
            ...
            $.ajax({
                url: "/society/delete_sc_upload_docs",
                data: form_data,
                type: 'POST',
                dataType : "json",
                contentType: false,
